...
1. **🛡️ Claude / GPT 타 AI 모델 토큰 100% 분리 필터링**:
   - Claude 3.5/3.7, GPT-4o, OpenAI, Anthropic 등 타 AI 모델 대화 로그를 식별하여 Gemini 토큰 수집에서 완전히 제외합니다.
   - Claude나 GPT 모델을 많이 사용하더라도 Gemini 잔여 쿼터(%)에 전혀 영향을 미치지 않으며, 오직 **Gemini 순수 소모량만 정밀 수집**합니다.

2. **📜 1~3일 세부 토큰 소모 로그 연동 (`token_history.json`)**:
   - 스캔된 모든 Gemini 대화 세션의 타임스탬프와 토큰 수치를 3일간(72시간) 자동으로 적재 관리하며, 72시간 이전 로그는 자동 정돈 삭제됩니다.

3. **⏰ 초기화 시점 수동 지정 & 이전 토큰 자동 차감 무시**:
...
2. **트레이 아이콘 사용**:
   - 트레이 아이콘 우클릭 $\rightarrow$ **[현 상태 보기]**로 Gemini 전용 쿼터 잔여량을 확인합니다.
